                ...
                <table class="table table-bordered table-striped mt-5">
                    ...
                    <tbody>
                        @foreach ($items as $index => $item)
                            <tr>
                                ...
                                <td><input type="number" wire:model="items.{{ $index }}.price" wire:change="itemUpdated({{ $index }})"
                                        class="form-control"></td>
                                <td><input type="number" wire:model="items.{{ $index }}.qty" wire:change="itemUpdated({{ $index }})"
                                        class="form-control"></td>
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <td><input type="number" wire:model="total" 
                                class="form-control" hidden>{{ $total }}</td>
                            <td></td>
                        </tr>
                    </tfoot>
                </table>
